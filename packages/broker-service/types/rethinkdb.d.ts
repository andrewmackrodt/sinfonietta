...
declare module 'rethinkdb' {
    ...
    export function table<T extends object, PKeyName extends string = 'id'>(name: string, options?: { useOutdated: boolean }): Table<T, PKeyName>
    ...
    export const maxval: Function

    type ValueExpression<T> =
        T extends object ? ObjectExpression<T> :
        T extends [] ? { [key: number]: ValueExpression<T> } :
        T

    export type ObjectExpression<T> = {
        [K in keyof T]: ValueExpression<T[K]> | Expression<any>
    }

    type InsertExpression<T, PKeyName extends string = 'id'> = ObjectExpression<T> | Omit<ObjectExpression<T>, PKeyName>

    type UpdateExpression<T, PKeyName extends string = 'id'> =
        { [K in keyof T]?: ValueExpression<T[K]> | Expression<any> } | Expression<any>

    // Control Structures
    export function branch<T>(
        test: Expression<boolean>,
        trueBranch: UpdateExpression<T>,
        falseBranch: UpdateExpression<T>,
    ): Expression<any>
    ...
    interface Db {
        ...
        table<T extends object, PKeyName extends string = 'id'>(name: string, options?: GetTableOptions): Table<T, PKeyName>
    ...
    interface Writeable<T extends object> {
        update(obj: UpdateExpression<T>, options: (UpdateOptions & { returnChanges: true })): Operation<ChangesWriteResult<T>>

        update(obj: UpdateExpression<T>, options?: UpdateOptions): Operation<WriteResult>

        replace(obj: UpdateExpression<T>, options: (UpdateOptions & { returnChanges: true })): Operation<ChangesWriteResult<T>>

        replace(obj: UpdateExpression<T>, options?: UpdateOptions): Operation<WriteResult>

        replace(expr: UpdateExpression<any>): Operation<WriteResult>

        delete(options: (UpdateOptions & { returnChanges: true })): Operation<DeleteWriteResult<T>>
    ...
    type InsertChangesOptions = InsertOptions & { returnChanges: true }

    type GetAllKey = string | number | boolean

    interface Table<T extends object, PKeyName extends string = 'id'> extends Sequence<T, PKeyName>, HasFields<Sequence<T, PKeyName>> {
        ...
        insert(obj: InsertExpression<T, PKeyName>, options: InsertChangesOptions): Operation<InsertChangesWriteResult<T>>

        insert(obj: InsertExpression<T, PKeyName>, options?: InsertOptions): Operation<InsertWriteResult>
         ...
         */
        getAll(...keys: GetAllKey[]): Sequence<T, PKeyName>

        /**
         * Get all documents matching a key on a simple index; defaults to primary key if no index provided.
         * See [getAll](https://www.rethinkdb.com/api/javascript/get_all/)
         */
        getAll(key: GetAllKey, index?: Index): Sequence<T, PKeyName> // without index defaults to primary key
        /**
         * Get all documents matching 2 or more keys on a simple index; defaults to primary key if no index provided.
         * See [getAll](https://www.rethinkdb.com/api/javascript/get_all/)
         */
        getAll(key: GetAllKey, key2: GetAllKey, index?: Index): Sequence<T, PKeyName> // without index defaults to primary key
        /**
         * Get all documents matching 2 or more keys on a simple index; defaults to primary key if no index provided.
         * See [getAll](https://www.rethinkdb.com/api/javascript/get_all/)
         */
        getAll(key: GetAllKey, key2: GetAllKey, key3: GetAllKey, index?: Index): Sequence<T, PKeyName> // without index defaults to primary key
        /**
         * Get all documents matching 2 or more keys on a simple index; defaults to primary key if no index provided.
         * See [getAll](https://www.rethinkdb.com/api/javascript/get_all/)
         */
        getAll(key: GetAllKey, key2: GetAllKey, key3: GetAllKey, key4: GetAllKey, index?: Index): Sequence<T, PKeyName> // without index defaults to primary key
        /**
         * Get all documents matching 2 or more keys on a simple index; defaults to primary key if no index provided.
         * See [getAll](https://www.rethinkdb.com/api/javascript/get_all/)
         */
        getAll(
            ...
            key5: GetAllKey,
            index?: Index,
        ): Sequence<T, PKeyName> // without index defaults to primary key
        /**
         ...
         */
        getAll(compoundKey: GetAllKey[], index: Index): Sequence<T, PKeyName> // without index defaults to primary key
        /**
         * Get all documents matching 2 or more compound index keys.
         * See [getAll](https://www.rethinkdb.com/api/javascript/get_all/) and
         * [compound indexes](https://www.rethinkdb.com/docs/secondary-indexes/javascript/#compound-indexes)
         */
        getAll(compoundKey: GetAllKey[], compoundKey2: GetAllKey[], index: Index): Sequence<T, PKeyName> // without index defaults to primary key
        /**
         * Get all documents matching 2 or more compound index keys.
         * See [getAll](https://www.rethinkdb.com/api/javascript/get_all/) and
         * [compound indexes](https://www.rethinkdb.com/docs/secondary-indexes/javascript/#compound-indexes)
         */
        getAll(
            compoundKey: GetAllKey[],
            compoundKey2: GetAllKey[],
            compoundKey3: GetAllKey[],
            index: Index,
        ): Sequence<T, PKeyName> // without index defaults to primary key
        /**
         * Get all documents matching 2 or more compound index keys.
         ...
         * [compound indexes](https://www.rethinkdb.com/docs/secondary-indexes/javascript/#compound-indexes)
         */
        getAll(key: Expression<any>, index?: Index): Sequence<T, PKeyName>
         ...
         * [compound indexes](https://www.rethinkdb.com/docs/secondary-indexes/javascript/#compound-indexes)
         */
        getAll(keys: Expression<any[]>, index: Index): Sequence<T, PKeyName>


        getIntersecting(geometry: Geometry, index: Index): Sequence<T, PKeyName>

        wait(WaitOptions?: WaitOptions): Operation<WaitResult>
    }

    interface Sequence<T extends object, PKeyName extends string = 'id'> extends Operation<Cursor>, Writeable<T> {
        between(lower: any, upper: any, index?: Index): Sequence<T, PKeyName>
        ...
        merge(object: Object): Sequence<T, PKeyName>

        merge(cb: ExpressionFunction<Expression<any>>): Sequence<T, PKeyName>

        filter(rql: ExpressionFunction<boolean>): Sequence<T, PKeyName>

        filter(rql: Expression<boolean>): Sequence<T, PKeyName>

        filter(obj: { [key: string]: any }): Sequence<T, PKeyName>
         ...
         * See: https://rethinkdb.com/api/javascript/changes/
         */
        changes(opts?: ChangesOptions): Sequence<T, PKeyName>

        // Join
        // these return left, right
        innerJoin(sequence: Sequence<T, PKeyName>, join: JoinFunction<boolean>): Sequence<T, PKeyName>

        outerJoin(sequence: Sequence<T, PKeyName>, join: JoinFunction<boolean>): Sequence<T, PKeyName>

        eqJoin(leftAttribute: string, rightSequence: Sequence<T, PKeyName>, index?: Index): Sequence<T, PKeyName>

        eqJoin(leftAttribute: ExpressionFunction<any>, rightSequence: Sequence<T, PKeyName>, index?: Index): Sequence<T, PKeyName>

        zip(): Sequence<T, PKeyName>

        // Transform
        map(transform: ExpressionFunction<any>): Sequence<T, PKeyName>

        withFields(...selectors: any[]): Sequence<T, PKeyName>

        concatMap(transform: ExpressionFunction<any>): Sequence<T, PKeyName>

        orderBy(...keys: string[]): Sequence<T, PKeyName>

        orderBy(...sorts: Sort[]): Sequence<T, PKeyName>

        skip(n: number): Sequence<T, PKeyName>

        limit(n: number): Sequence<T, PKeyName>

        slice(start: number, end?: number): Sequence<T, PKeyName>

        nth(n: number): Expression<any>

        indexesOf(obj: any): Sequence<T, PKeyName>

        isEmpty(): Expression<boolean>

        union(sequence: Sequence<T, PKeyName>): Sequence<T, PKeyName>

        sample(n: number): Sequence<T, PKeyName>

        getField(prop: string): Sequence<T, PKeyName>
        ...
        distinct(opts?: { index: string }): Sequence<T, PKeyName>

        groupedMapReduce(group: ExpressionFunction<any>, map: ExpressionFunction<any>, reduce: ReduceFunction<any>, base?: any): Sequence<T, PKeyName>

        groupBy(...aggregators: Aggregator[]): Expression<Object> // TODO: reduction object
        contains(prop: string): Expression<boolean>

        // Manipulation
        pluck(...props: string[]): Sequence<T, PKeyName>

        without(...props: string[]): Sequence<T, PKeyName>
    }
    ...
    interface InsertWriteResult extends WriteResult {
        generated_keys: string[]
    }

    interface InsertChangesWriteResult<T extends object> extends InsertWriteResult {
        changes?: { old_val: object; new_val: T }[]
    }

    interface ChangesWriteResult<T extends object> extends WriteResult {
        changes: { old_val: T; new_val: T }[]
    }

    interface DeleteWriteResult<T extends object> extends WriteResult {
        changes?: { old_val: T; new_val: null }[]
    }
    ...
    // TODO add Writeable generic
    interface Expression<T> extends Writeable<any>, Operation<T>, HasFields<Expression<number>> {
        (prop: string): Expression<any>

        merge(query: Expression<Object>): Expression<Object>

        append(prop: string | object): Expression<Object>
